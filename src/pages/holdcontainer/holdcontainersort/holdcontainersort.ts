...
@IonicPage()
@Component({
  selector: 'page-holdcontainersort',
  templateUrl: 'holdcontainersort.html',
  providers: [Utils,SortModel]
})

export class HoldContainerSortPage {
  sortMode: string;
  sort : boolean = false;
  sortselect : string;
  ...
  constructor(public navParams: NavParams,public utils:Utils, public viewCtrl: ViewController,public navCtrl: NavController,) {
    this.sortselect = 'chholdrequestno';
    this.sortMode = "Descending";

    this.rchSortModal = this.navParams.get("sortModal");
    if(this.rchSortModal.sortOrder || this.rchSortModal.sortOption) {
